...
set :bpl_ruby_version, File.read(File.expand_path('./../.ruby-version', __dir__)).strip
...
namespace :boston_library do
  ...
  task :gem_update do
    on roles(:app) do
      execute("#{fetch(:rvm_installed)} #{fetch(:bpl_ruby_version)} do gem update --system --no-document")
    end
  end
  ...
  task :rvm_install_ruby do
    on roles(:app) do
      execute("#{fetch(:rvm_installed)} install #{fetch(:bpl_ruby_version)} -C --with-jemalloc")
      execute("#{fetch(:rvm_installed)} use #{fetch(:bpl_ruby_version)}")
    end
  end
  ...
  task :install_bundler do
    on roles(:app) do
      execute("#{fetch(:rvm_installed)} #{fetch(:bpl_ruby_version)} do gem install bundler:#{fetch(:rvm_bundle_version)}")
    end
  end
